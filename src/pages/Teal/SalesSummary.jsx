...
console.log(todaysItems)
  ...
  console.log(summary)
const expensesListLines = summary.expensesList
  ? summary.expensesList.split('\n')
  : []
const repairsListLines = summary.repairsList
  ? summary.repairsList.split('\n')
  : []
        ...
        <div ref={componentRef} className='m-4'>
          ...
          <div className='row'>
            <div className='col-sm'>
              ...
              <p>Cash</p>
              <p>Today's Sale</p>
              <p>{summary.totalPhoneSale && 'Phone Sales'}</p>
              <p>Sales By Model</p>
              <p>{summary.phones && 'Phones Sold'}</p>
              <p>{summary.repairs && 'Repairs'}</p>
              <p>{summary.expenses && 'Expenses'}</p>
              <p>{summary.expensesList && 'Remarks'}</p>
            </div>
                ...
                <tbody>
                  {todaysItems.length > 0 ? (
                    todaysItems.map((item) => (
                      <tr key={item.id}>
                        <td>
                          {item.model}- {item.color} ({item.condition})
                        </td>
                        <td>
                          {!item.taxIn
                            ? (item?.soldPrice / 1.13).toFixed(2)
                            : item.soldPrice}
                        </td>
