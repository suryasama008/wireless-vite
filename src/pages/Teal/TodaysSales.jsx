...
<td className='text-center col-sm-3 '>
  <input 
    className='form-control text-center' 
    disabled 
    value={(parseFloat(summary?.card) + (parseFloat(summary?.cash))).toFixed(2)}
  />
</td>
</tr>
<tr>
...
<tr>
          ...
          {items.map((item) => (
            <tr key={item.id}>
              ...
              <td>
                ...
                onChange={(e) => onPaymentMethodChange(item.id, e.target.value)}
                >
              ...
              <td>
                <Button color='danger' className='btn-sm' onClick={() => handleRemoveItem(item.id)}>
                  Remove
                </Button>
              </td>
            </tr>
    ...
    <div className='text-center d-flex justify-content-between px-4'>
      <Button color = 'primary' onClick = {onSave} >Submit</Button>
      <Button color = 'success' onClick = {onSendEmail} >Send Report</Button>
    </div>
    </CardBody>
    </Card>
    </div>
