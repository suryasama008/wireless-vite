...
// const items = useSelector((state) => state.ItemReducer.items)/
  ...
  // Usage
  const updatedDate = new Date(); // Replace with your date object
  const formattedDate = formatDate(updatedDate);
  console.log()

  const fetchProducts = async () => {
    const currentDate = moment().format('DD-MM-YYYY');
    const prodQuery = query(prodCollectionRef, where('updatedDate', '==', '17-12-2023'), where('store', '==', user.store) );
    const soldQuery = query(prodCollectionRef, where('soldDate', '==', '17-12-2023'),where('store', '==', user.store));
  
    try {
      const [addData, soldData] = await Promise.all([getDocs(prodQuery), getDocs(soldQuery)]);

      const addProd = addData.docs.map(doc => ({ ...doc.data(), id: doc.id }));
      const soldProd = soldData.docs.map(doc => ({ ...doc.data(), id: doc.id }));
  
      // Combine and remove duplicates
      const combinedProducts = [...addProd, ...soldProd];
          ...
          <ul className='list-unstyled hstack gap-1 mb-0'>
            ...
            <li>
              ...
              >
               ...
               <Button className='btn btn-sm btn-soft-primary' onClick = {()=> {
            ...
            <Row className='justify-content-between align-items-start'>
              ...
              <Col xs={4} className='text-end'>
                <div className='flex-shrink-0'>
                  ...
                  <Button color ='primary' onClick = {()=> setmodal_standard(true)}>Add New Item</Button>
                </div>
              </Col>
            </Row>
          </CardBody>
