...
const Order = () => {
  ...
  const [addToCartCheck, setAddToCartCheck] = useState({})
  ...
  const addToCartOrders = (order) => {
    ...
    //      : order.cases
    order.cases =
      order.cases === 'SILICONE'
        ? `SILICONE-${siliconeColors[`${order.prodId}-${order.cases}`]}`
        : order.cases
    order.quantity = quantities[`${order.prodId}-${order.cases}`]
    order.addToCartCheck = addToCartCheck[`${order.prodId}-${order.cases}`]
    setCartOrders((prevCartOrders) => {
  ...
  function handleAddToCartCheck(id, value, cases) {
    setAddToCartCheck((prevAddToCartCheck) => ({
      ...prevAddToCartCheck,
      [`${id}-${cases}`]: value,
    }))
  }
  ...
  const models = [
    { brand: 'SAMSUNG', models: ['S22', 'S23'] },
    { brand: 'APPLE', models: ['IPHONE XR', 'IPHONE XS MAX', 'IPHONE 13'] },
  ]
  ...
  const columns = [
    ...
    },
    {
      ...
      label: 'ADD TO CART',
      options: {
        customBodyRender: (data, dataIndex, updateValue) => {
          // Check if the product is in the cartOrders
          const id = dataIndex.rowData[6]
          const cases = dataIndex.rowData[3]
          const check = addToCartCheck[`${id}-${cases}`] || false
          ...
          const handleChange = (event) => {
            ...
            setAddToCartCheck((prevAddToCartCheck) => ({
              ...prevAddToCartCheck,
              [`${id}-${cases}`]: event.target.checked, // Replace 'value' with 'event.target.checked'
            }))
            handleAddToCartCheck(id, event.target.checked, cases)
...
const SiliconeColor = ({ setSiliconeColor, handleColorChange, id, cases }) => {
