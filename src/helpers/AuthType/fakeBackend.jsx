...
const fakeBackend = () => {
  ...
  mock.onPost(url.ADD_NEW_ITEM).reply((item) => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (item && item.data) {
          ...
          reject([400, 'Cannot add item'])
        }
      })
    })
  })

 mock.onPut(url.UPDATE_ITEM).reply((item) => {
   return new Promise((resolve, reject) => {
     setTimeout(() => {
       if (item && item.data) {
         // Passing fake JSON data as response
         ...
         reject([400, 'Cannot update item'])
       }
     })
   })
 })

  mock.onDelete(url.DELETE_ITEM).reply((config) => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (config && config.headers) {
          // Passing fake JSON data as response
          resolve([200, config.headers.item])
        } else {
          reject([400, 'Cannot delete item'])
        }
      })
    })
  })
...
mock.onGet(url.GET_ITEMS).reply(() => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      if (items) {
        // Passing fake JSON data as response
